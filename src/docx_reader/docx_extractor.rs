use crate::docx_reader::convert::extract_text_from_table_row_content;
...
pub fn extract(extract_from_docx: &PathBuf) -> ExtractContainer {
    ...
    for (index, header) in table.rows[0].cells.iter().enumerate() {
        let text = crate::docx_reader::convert::extract_text_from_table_row_content(header);
    ...
    for table_row in table.rows.iter().skip(1) {
        ...
        let variation = PluralVariate::from_android_key(&extract_text_from_table_row_content(
            &table_row.cells[index_variation],
        ));
